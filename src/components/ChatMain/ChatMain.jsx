...
const ChatMain = () => {
  ...
  const getGroupedMessages = (messages) => {
    ...
    messages.forEach(msg => {
      const fixedDate = fixTimestamp(msg.timestamps); // ← usar hora corregida
      const msgDateStr = formatDateToLocalString(fixedDate); // ← fecha YYYY-MM-DD en Colombia
  
      if (lastDateStr !== msgDateStr) {
        const separatorText = getSeparatorText(fixedDate); // ← usa la hora corregida
      ...
      grouped.push({
        ...msg,
        type: 'message',
        timestamps: fixedDate.toISOString(), // o podés pasar `fixedDate` directo si lo necesitás como Date
      });
    });
